struct SequenceForm {
    ...
    payoffs: HashMap<Player,Vec<BigRational>>,
    constraints: HashMap<Player,Vec<i64>>,
    seqin: HashMap<InformationSet,Move>,
    defseq: HashMap<Node,HashMap<Player,Move>>,
    seqs: HashMap<Player,Vec<Move>>,
    seqs_index: HashMap<Move,usize>,

    isets: Vec<InformationSet>,
    isets_by_pl: HashMap<Player,Vec<InformationSet>>,
    iset_indices: HashMap<InformationSet,usize>,
    rand_priors: HashMap<Move,BigRational>,
}

impl SequenceForm {

    //TODO: private Player firstPlayer;

    //static int oldnseqs1 = 0;
    //static int[] oldconstrows = new int[] {0, 0, 0};
    fn new(extensive_form: ExtensiveForm, seed: i64) -> SequenceForm {
    	_seed = seed;
    ...
    fn genseqin(extensive_form: &ExtensiveForm) {
    	Random prng = null;
    	if (_seed != null) prng = new Random(_seed);
    	recgenseqin(tree.root(), prng);
    }

    fn recgenseqin(node: &Node, Random prng) {
    ...
    fn seq_count_for_pl(&self, pl: Player) -> usize {
    	return seqsMap.get(pl).size();
    }

    fn iset_count_for_pl(&self, pl: Player) -> usize {
    	match self.isets_for_pl.get(pl) {
            Some(isets) => isets.len(),
            None => 0
        }
    }
	...
	fn rec_expected_payoffs(node: &Node, Rational prob, Map<Player, Map<Move, Rational>> equilib, Map<Player, Rational> epayoffs) {
		if (node.outcome != null) {
			...
			for (Node child = node.firstChild(); child != null; child = child.sibling()) {
				if (pl == Player.CHANCE) {
					rec_expected_payoffs(child, prob.multiply(child.reachedby.prob), equilib, epayoffs);
				} else if (equilib.get(pl).containsKey(child.reachedby)) {
					rec_expected_payoffs(child, prob.multiply(equilib.get(pl).get(child.reachedby)), equilib, epayoffs);
				}
			}
		}
	}
